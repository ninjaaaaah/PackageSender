        ...
        while True:
            ...
            try:
                ...
                if last != 0 and last != initsize:
                    ...
                    size = int(size // (5/2))
                print(
                    f"NON | LEN: {str(size).zfill(3)} | DUR: {duration:2.2f} | COM: {sent}/{len(data)}")
                elapsed += duration
                cons += 1
                if cons == 3:
                    break
            ...
            if self.verifyAck(seqID, ack, packet):
                print(
                    f"ACK | LEN: {str(size).zfill(3)} | DUR: {duration:2.2f} | COM: {sent+size}/{len(data)}")
            else:
                print(
                    f"ERR | LEN: {str(size).zfill(3)} | DUR: {duration:2.2f} | COM: {sent+size}/{len(data)}")
